...
func main() {
	...
	for s.Scan() {
		txt := s.Text()
		gameID, colonIndex := getGameID(txt)
		gameData := strings.Split(txt[colonIndex+2:], "; ")

		maxColors := make(map[string]int)
		for _, g := range gameData {
			g = strings.ReplaceAll(g, " ", "")
			round := strings.Split(g, ",")
			for _, game := range round {
				number, color := numAndColor(game)
				if maxColors[color] < number {
					maxColors[color] = number
				}
			}
		}
		if maxColors[colorRed] <= maxRed && maxColors[colorBlue] <= maxBlue && maxColors[colorGreen] <= maxGreen {
			sumIDs += gameID
		}
	}
	fmt.Println(sumIDs)
...
func numAndColor(s string) (int, string) {
	var num string
	var color string
	for i, c := range s {
...
func getGameID(s string) (int, int) {
	numStr := ""
	var colonIdx int
	for i := 5; ; i++ {
		if s[i] == ':' {
...
var input = `Game 1: 14 green, 8 blue, 9 red; 5 blue, 4 green, 2 red; 4 red, 4 blue, 4 green; 1 blue, 3 green, 2 red; 10 red, 3 blue, 15 green; 2 red, 6 green, 3 blue
